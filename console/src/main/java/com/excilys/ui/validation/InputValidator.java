package com.excilys.ui.validation;
...
public interface InputValidator {
  ...
  public static void isInt(String input) {
    ...
    if (!matcher.find()) {
      throw new IllegalArgumentException("The given input isn't an int " + input);
    }
  }
  ...
  public static void isDate(String input) {
    Pattern pattern = Pattern.compile("[0-1][1-9]-[0-3][0-9]-[0-9]{4}");
    Matcher matcher = pattern.matcher(input);

    if (!matcher.find()) {
      throw new IllegalArgumentException(
          "The given input isn't a date (format YYYY-MM-DD)" + input);
    }
  }
